...
//   4. HomeShell            -> tela principal (velocímetro + dock + status bar)
//   5. SpeedometerGauge     -> CustomPainter alimentado pelo GPS real (geolocator)
//   6. WingLogoPainter      -> identidade visual geométrica (linhas de asa)
                ...
                Column(
                  children: [
                    ...
                    Expanded(
                      child: Row(
                        children: [
                          // Logo geométrica discreta (linhas de asa) no canto.
                          SizedBox(
                            width: 90,
                            child: Center(
                              child: CustomPaint(
                                size: const Size(64, 64),
                                painter: WingLogoPainter(color: neon),
                              ),
                            ),
                          ),
                          // Velocímetro central.
                          const Expanded(child: Center(child: SpeedometerGauge())),
...
// ---------------------------------------------------------------------------
// 5) IDENTIDADE VISUAL — linhas fluidas inspiradas em asa de avião
// ---------------------------------------------------------------------------

class WingLogoPainter extends CustomPainter {
  WingLogoPainter({required this.color});
  final Color color;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      canvas.drawPath(path, paint..color = color.withOpacity(0.9 - i * 0.2));
    }
  }

  @override
  bool shouldRepaint(covariant WingLogoPainter oldDelegate) => oldDelegate.color != color;
}
      ...
      body: Column(
        children: [
          Container(
            ...
            child: Text(
              mapFolder == null
                  ? 'Usando tiles embutidos no APK (padrão)'
